...
import {
  getPaymentsStatusService,
  paymentStatusService,
} from "../../services/payment/paymentStatusService.js";
...
export const getPaymentsStatusController = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    const paymentsStatus = await getPaymentsStatusService();
    res.status(200).json({ paymentsStatus });
  } catch (error) {
    console.error(error);
    next(error);
  }
